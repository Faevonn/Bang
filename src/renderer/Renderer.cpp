...
// Raw Vulkan, no helper library (no VMA, no vk-bootstrap). Impl struct
// holds every Vulkan handle; look at the createX()/destroyX() method pairs
// in order (createInstance, pickDevice, createSwapchain, createAtlas,
// createPipeline) to follow the setup sequence, it mirrors a typical
// Vulkan tutorial's init order. Shaders are precompiled SPIR-V embedded at
// build time (see quad_vertex.spv.hpp/quad_fragment.spv.hpp, generated by
// cmake/embed_spirv.cmake from shaders/quad.vert/.frag), so editing the
// GLSL requires a rebuild to regenerate those headers, editing the .hpp
// files directly does nothing.
